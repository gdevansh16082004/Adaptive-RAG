...
with st.sidebar:
        ...
        if file_description:
            if file_key not in st.session_state.uploaded_files:
                ...
                success = document_upload_rag(
                    uploaded_file, file_description, st.session_state["username"]
                )
                if success:
    ...
    # List the user's indexed documents with delete actions
    st.header("📄 Your Documents")
    username = st.session_state.get("username")
    if username:
        documents = list_documents(username)
        if not documents:
        ...
        for doc in documents:
            ...
            if col_del.button("🗑️", key=f"del_{doc['doc_id']}"):
                if delete_document(doc["doc_id"], username):
                    st.rerun()
                else:
