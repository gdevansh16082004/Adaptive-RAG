...
## 🔄 Step 5: Point the App at Your Cluster

**The code switch is already done** — the app now talks to Qdrant directly
(`src/db/qdrant_client.py`, `src/rag/retriever_setup.py`). No FAISS remains.
You only configure `.env` (copy `.env.example`):

```env
# Local Docker
QDRANT_URL=http://localhost:6333

# ...or Qdrant Cloud (URL must include https:// and the port)
QDRANT_URL=https://your-cluster-id.aws-us-east-1.cloud.qdrant.io:6333
QDRANT_API_KEY=your-api-key-here

QDRANT_DOCS_COLLECTION=documents
```

Start a local Qdrant with Docker if you prefer no cloud account:
...
curl http://localhost:8000/health   # → {"status":"ok","qdrant":"ok","mongo":"ok"}
```

The `documents` collection and its payload indexes are created automatically
